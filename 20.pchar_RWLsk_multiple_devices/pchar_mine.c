...
#define DEV_CLASS_NAME "vz_class"
 ...
 //DEFINE_KFIFO(pchar_fifo, char, MAXLEN);
...
static struct file_operations pchar_fops = {  //tagged structure initilization
    ...
    .read = pchar_read,
    .write = pchar_write,
   // .llseek=pchar_lseek //make note of name is llseek. LSEEK does not make any effect in KFIFO
};
...
int __init pchar_init(void)
{
    ...
    //Step 0 allocate memory for private struct
        ...
        printk("***** kmalloc succeed for %d devices",devices);
...
//	avail = MAXLEN - *pf_pos;
    avail = kfifo_len(&pchar_fifo);
    bytes_to_read = len < avail ? len : avail;
	if(bytes_to_read <= 0) {
		printk("[%s]: pchar_read() reached to end of device.\n", THIS_MODULE->name);
		return 0;
	}
	//nbytes = bytes_to_read - raw_copy_to_user(buf, pchar_fifo + *pf_pos, bytes_to_read); // used from asm/uaccess.h
    ret = kfifo_to_user(&pchar_fifo, buf, bytes_to_read, &nbytes);
    if(ret!=0)
        {
            printk("!!!!!!!!!!!!!!!! kfifo is failed to read kernel buffer");
            return -ENOSPC;
        }
...
//pf_pos -> pointer to file position
 static ssize_t pchar_write(struct file *pfile, const char __user *buf, size_t len, loff_t *pf_pos)
 {
	...
	//avail = MAXLEN - *pf_pos; //*** Replaced with below kfifo function
    
    avail = kfifo_avail(&pchar_fifo);
	...
	//nbytes = bytes_to_write - raw_copy_from_user(pchar_fifo + *pf_pos, buf, bytes_to_write);   //used from asm/uaccess.h   *** Replaced with below kfifo function
    ret = kfifo_from_user(&pchar_fifo, buf, bytes_to_write, &nbytes);
        //copy from user and kfifo_in

    if(ret!=0)
        {
            printk("!!!!!!!!!!!!!!!! kfifo is failed to write kernel buffer");
            return -ENOSPC;
        }
